...
def schreibeInDatei(datei_name, energie, MPV, STDV,einheit,teilchen):
    ...
    try:
        ...
        with open(datei_name, 'w+') as f_obj:
            f_obj.write("#  Energie/ MeV\t MPV\t STDV\n" +"\t" +str(energie) +"\t" +str(MPV) +"\t" +str(STDV) + "\t"+einheit+"\t"+teilchen+"\n")
    else:
        with open(datei_name, 'a') as f_obj:
            f_obj.write(str(energie) +"\t" +str(MPV) +"\t" +str(STDV) +"\t"+ einheit+ "\t"+teilchen+ "\n")

#newpath = r'Test'
path = sys.argv[1] #Ordner
dirs = os.listdir( path )

filename = sys.argv[2] #Ordner
...
for dateien in dirs:
    if dateien[-5:]==".root":
        ...
        energie = name_zerlegt[3] # 4 for Ar!!
        teilchen = name_zerlegt[2]
        einheit = name_zerlegt[4]
#reed a histogram
        ...
        # #func.Print()
        schreibeInDatei(filename, energie, MPV, ERR,einheit,teilchen)
        func.Draw("SAME")
        c1.Update()
